...
final class CoversCarouselControl: NSObject {
    fileprivate let presenter: CoversPresenter
    
    init(presenter: CoversPresenter, carouselView: iCarousel?) {
...
extension CoversCarouselControl: iCarouselDataSource {
    func numberOfItems(in carousel: iCarousel) -> Int {
        return presenter.getCoverListCount()
    }
    ...
    func carousel(_ carousel: iCarousel, viewForItemAt index: Int, reusing view: UIView?) -> UIView {
        ...
        imageView.image = UIImage(named: presenter.getCoverImageView(at: index))
        titleLabel.text = presenter.getCoverName(at: index)
        return itemView
    }
}
...
extension CoversCarouselControl: iCarouselDelegate {
    func carousel(_ carousel: iCarousel, didSelectItemAt index: Int) {
        presenter.carouselSelected(index)
    }
    
    func carouselCurrentItemIndexDidChange(_ carousel: iCarousel) {
        presenter.carouselChanged(carousel.currentItemIndex)
    }
}
